...
int search(PuzzleState<MoveType>& current, const PuzzleState<MoveType>& goal, int g, int threshold, int maxDepth);
...
void idaStar(std::unique_ptr<PuzzleState<MoveType>> start, const PuzzleState<MoveType>& goal, int maxDepth) {
    ...
    while (threshold <= maxDepth) {  // Continue only within the maximum depth
        int temp = search(*start, goal, 0, threshold, maxDepth);
        if (temp == -1) {
...
int search(PuzzleState<MoveType>& current, const PuzzleState<MoveType>& goal, int g, int threshold, int maxDepth) {
    ...
    if (current.isGoal(goal)) {
        //current.printState();
        std::cout << "Solution found with " << g << " moves:\n";
        current.printMoves();
        std::cout << std::endl;
    ...
    for (MoveType move : current.getValidMoves()) {
        ...
        int temp = search(*next, goal, g + 1, threshold, maxDepth);
        if (temp == -1) {
